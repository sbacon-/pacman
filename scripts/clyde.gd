...
func _process(_delta: float) -> void:
	if (GameManager.round_number == 1 && GameManager.dots_eaten > 90):
		await get_tree().create_timer(0.5).timeout
		can_escape = true
	elif (GameManager.round_number == 2 && GameManager.dots_eaten > 50):
		await get_tree().create_timer(0.5).timeout
		can_escape = true
	elif (GameManager.round_number >=3):
		await get_tree().create_timer(0.5).timeout
		can_escape = true
...
func chase_movement() -> void:
	if current_tile_center().distance_to(player.current_tile_center()) < 64:
		set_target_position(scatter_pos)
	else:
		target.global_position = player.current_tile_center()
